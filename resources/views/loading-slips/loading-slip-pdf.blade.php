...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: DejaVu Sans, sans-serif; font-size: 11px; line-height: 1.35; color: #111; margin: 0; }
        ...
        .company-name { font-size: 18px; font-weight: bold; font-family: DejaVu Serif, serif; color: #1e3a8a; margin-top: 6px; }
        .slip-title { font-size: 22px; font-weight: bold; color: #1d4ed8; letter-spacing: 1px; text-align: right; }
        .title-rule { border-bottom: 2px solid #1d4ed8; margin-top: 4px; margin-bottom: 8px; }
        .date-box { border: 1px solid #333; margin-left: auto; width: 200px; }
        .date-box td { border: 1px solid #333; padding: 4px 8px; font-size: 10px; }
        .date-box .label { font-weight: bold; width: 45%; }
        .office-address { text-align: center; font-size: 10px; margin: 10px 0 12px; line-height: 1.5; }
        .route-bar { background: #1e3a8a; color: #fff; font-weight: bold; font-size: 12px; padding: 6px 10px; }
        .route-row td { border: none; padding: 0; width: 50%; }
        .route-meta { font-size: 11px; padding: 6px 4px 10px; }
        .lines-table th { background: #1e3a8a; color: #fff; font-size: 10px; text-align: center; border: 1px solid #1e3a8a; padding: 6px 4px; }
        .lines-table td { border: 1px solid #333; text-align: center; padding: 6px 4px; }
        .lines-table td.num { text-align: right; }
        .lines-table tfoot td { font-weight: bold; background: #f3f4f6; }
        ...
        .summary-box td { border: 1px solid #333; padding: 6px 10px; font-weight: bold; font-size: 11px; }
        .summary-box .label-cell { background: #d1fae5; }
        .summary-box .value-cell { background: #fef08a; text-align: right; }
        .sign-block { text-align: right; font-size: 11px; }
        .sign-block .sign-title { font-weight: bold; margin-bottom: 24px; }
    </style>
</head>
<body>
@php
    $display = static fn ($value) => ($value === null || $value === '' || (is_string($value) && trim($value) === '')) ? '—' : (string) $value;
    ...
    <tr>
        ...
        <td class="title-cell">
            ...
            <table class="date-box">
                <tr><td class="label">DATE</td><td>{{ $formatDate($slip->slip_date) }}</td></tr>
                <tr><td class="label">Loading date</td><td>{{ $formatDate($slip->loading_date) }}</td></tr>
            </table>
        </td>
    </tr>
</table>
...
<table class="route-row">
    <tr>
        ...
        <td>
            ...
            <div class="route-meta">@if ($contactEmail) E-Mail : {{ $contactEmail }} @endif</div>
        </td>
    </tr>
</table>
...
<table class="lines-table">
    ...
    <tbody>
        ...
        @empty
            <tr><td colspan="6">No vehicles listed.</td></tr>
        @endforelse
    </tbody>
    @if ($lines->isNotEmpty())
        <tfoot>
    ...
    <tr>
        <td style="width: 55%;">
            <div class="summary-box">
                <table>
                    <tr><td class="label-cell">TOTAL FREIGHT</td><td class="value-cell">{{ $formatMoney($slip->total_freight) }}</td></tr>
                    <tr><td class="label-cell">TOTAL ADVANCE</td><td class="value-cell">{{ $formatMoney($slip->total_advance) }}</td></tr>
                </table>
            </div>
        </td>
        <td style="width: 45%;">
