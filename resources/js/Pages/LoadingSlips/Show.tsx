...
export default function LoadingSlipShow({ loadingSlipId }: { loadingSlipId: number }) {
    ...
    const refresh = async () => {
        ...
        try {
            ...
            if (!res.success || !res.data?.loadingSlip) {
                setError(res.message || 'Could not load loading slip.');
                return;
            }
            ...
            setError('Could not load loading slip.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownload = async () => {
        ...
        try {
            await downloadLoadingSlipPdf(slip.id, slip.slip_date.slice(0, 10));
        } catch {
            setActionError('Could not download PDF.');
        } finally {
    ...
    const handleGenerateInvoice = async () => {
        ...
        try {
            const res = await appApiPost<
                ApiEnvelope<{ invoice: { id: number }; loadingSlip: LoadingSlip }>
            >('/loading-slips/loading-slip-generate-invoice', { id: loadingSlipId });

            if (!res.success || !res.data?.invoice) {
                setActionError(res.message || 'Could not generate invoice.');
                return;
            }

            router.visit(route('invoices.show', res.data.invoice.id));
        } catch {
            setActionError('Could not generate invoice.');
        } finally {
            setGenerating(false);
        }
    };
                ...
                {loading && !slip ? (
                    ...
                    <>
                        <div className="flex flex-wrap items-center gap-3">
                            <SecondaryButton type="button" onClick={() => void handleDownload()} disabled={downloading}>
                                {downloading ? 'Downloading…' : 'Download PDF'}
                            </SecondaryButton>
                            {slip.status === 'draft' && (
                                <>
                                    <Link href={route('loading-slips.edit', slip.id)}>
                            ...
                            {slip.freight_invoice && (
                                <Link
                                    href={route('invoices.show', slip.freight_invoice.id)}
                        ...
                        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                            <div className="border-b border-gray-100 px-6 py-5">
                                    ...
                                    <div>
                                        {company?.logo_url && (
                                            <img
                                                ...
                                                alt=""
                                                className="mb-2 max-h-14 max-w-[120px] object-contain"
                                            />
                                        )}
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="mt-2 inline-block border border-gray-300 text-left text-sm">
                                            <div className="grid grid-cols-2 border-b border-gray-300">
                                                <span className="border-r border-gray-300 px-3 py-1 font-semibold">
                                                    DATE
                                                </span>
                                                <span className="px-3 py-1">{formatSlipDate(slip.slip_date)}</span>
                                            </div>
                                            <div className="grid grid-cols-2">
                                                <span className="border-r border-gray-300 px-3 py-1 font-semibold">
                                                    Loading date
                                                </span>
                                                <span className="px-3 py-1">{formatSlipDate(slip.loading_date)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {officeAddress && (
                                    <p className="mt-4 whitespace-pre-line text-center text-sm text-gray-700">
                                        {officeAddress}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2">
                                <div>
                                    <div className="bg-blue-900 px-4 py-2 text-sm font-bold uppercase text-white">
                                        FROM: {slip.route_from?.toUpperCase() || '—'}
                                    </div>
                                    <p className="px-4 py-3 text-sm">
                                        M/S:{' '}
                                        {slip.party ? (
                                            <PartyLink
                                                partyId={slip.party.id}
                                                name={slip.party.name}
                                ...
                                <div>
                                    <div className="bg-blue-900 px-4 py-2 text-sm font-bold uppercase text-white">
                                        TO: {slip.route_to?.toUpperCase() || '—'}
                                    </div>
                                    {contactEmail && (
                                        <p className="px-4 py-3 text-sm">E-Mail : {contactEmail}</p>
                                    )}
                                </div>
                            </div>
                                ...
                                <table className="min-w-full border-collapse text-sm">
                                    <thead>
                                        <tr className="bg-blue-900 text-white">
                                            ...
                                            <th className="border border-blue-900 px-3 py-2 text-right">
                                                Freight rate
                                            </th>
                                    ...
                                    <tbody>
                                        {lines.map((line) => (
                                            <tr key={line.serial_number} className="text-center">
                                                ...
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(line.freight_rate)}
                                                </td>
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(line.advance)}
                                                </td>
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(line.balance)}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    {lines.length > 0 && (
                                        <tfoot>
                                            <tr className="bg-gray-50 font-semibold">
                                                ...
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(slip.total_freight)}
                                                </td>
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(slip.total_advance)}
                                                </td>
                                                <td className="border border-gray-300 px-3 py-2 text-right">
                                                    {formatSlipAmount(slip.total_balance)}
                                                </td>
                                            </tr>
                                        </tfoot>
                                    )}
                            ...
                            <div className="grid grid-cols-1 gap-6 px-6 py-5 md:grid-cols-2">
                                <div className="max-w-xs">
                                    <table className="w-full border-collapse text-sm">
                                        <tbody>
                                            <tr>
                                                <td className="border border-gray-300 bg-emerald-100 px-3 py-2 font-bold">
                                                    TOTAL FREIGHT
                                                </td>
                                                <td className="border border-gray-300 bg-yellow-200 px-3 py-2 text-right font-bold">
                                                    {formatSlipAmount(slip.total_freight)}
                                                </td>
                                            </tr>
                                            <tr>
                        ...
                        <div className="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-600">
                            Totals: freight {formatMoney(slip.total_freight)}, advance{' '}
                            {formatMoney(slip.total_advance)}, balance {formatMoney(slip.total_balance)}
                        </div>
                    </>
                ) : null}
            </PageContainer>
