...
type LoadingSlipFilters = ListFilters & {
    party_id?: string;
    status?: string;
};
...
const defaultFilters: LoadingSlipFilters = {
    search: '',
    party_id: '',
    status: '',
    ...defaultDateFilters,
...
function statusLabel(status: LoadingSlip['status']): string {
    return status === 'invoiced' ? 'Invoiced' : 'Draft';
}
            ...
            <PageContainer className="space-y-4">
                ...
                {loading && !data ? (
                    ...
                    <ListingTableShell
                        ...
                        emptyMessage={
                            hasActiveFilters
                                ? 'No loading slips match your filters.'
                        ...
                        mobile={slips.map((slip, index) => (
                            <ListingMobileCard
                                ...
                                subtitle={
                                    <PartyLink
                                        partyId={slip.party_id ?? slip.party?.id}
                                        name={slip.party?.name}
                                    />
                                }
                                metric={{
                                    label: 'Freight',
                                    value: `₹ ${formatMoney(slip.total_freight)}`,
                                }}
                                fields={[
                                    {
                                        label: 'Route',
                                        value:
                                            [slip.route_from, slip.route_to].filter(Boolean).join(' → ') ||
                                            '—',
                                    },
                                    {
                                        label: 'Advance',
                                        value: `₹ ${formatMoney(slip.total_advance)}`,
                                    },
                                    {
                                        label: 'Status',
                                        value: statusLabel(slip.status),
                                    },
                                ]}
                                actions={
                                    <>
                                        <ListingMobileAction
                                            href={route('loading-slips.show', slip.id)}
                                            variant="primary"
                                        >
                                            View
                                        </ListingMobileAction>
                                        {slip.status === 'draft' && (
                                            <ListingMobileAction
                                                href={route('loading-slips.edit', slip.id)}
                                            >
                                                Edit
                                            </ListingMobileAction>
                                        )}
                                ...
                                {slips.map((slip) => (
                                    <tr key={slip.id} className="hover:bg-gray-50">
                                        <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">
                                            {formatAppDateTime(slip.slip_date, '—')}
                                        </td>
                                        ...
                                        <td className="whitespace-nowrap px-4 py-3 text-right text-sm text-gray-900">
                                            {formatMoney(slip.total_freight)}
                                        </td>
                                        <td className="whitespace-nowrap px-4 py-3 text-right text-sm text-gray-900">
                                            {formatMoney(slip.total_advance)}
                                        </td>
